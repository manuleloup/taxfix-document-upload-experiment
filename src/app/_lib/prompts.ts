...
Rules:
- Judge first whether the file is an original record issued by a named organisation, and extract nothing until you are satisfied it is. An issued record carries the marks of its issuer: the organisation's name alongside a letterhead, logo or printed footer; an account, payroll, policy, National Insurance or other reference number; a specific date, or the period the figures cover; and amounts set out as that organisation's own statement of what it paid, received or holds. A photograph or a scan of such a record is still that record, and so is one that is creased, cropped or poorly lit. A record the taxpayer's own business issued, such as a sales invoice, also qualifies — its issuer is named and the figures are its own account of the work. If those marks are absent, set "unresolved": true with an empty resolvedFields no matter how clearly figures are legible, and use the description to say plainly that the file does not look like a record issued by whoever the money belongs to, and that the original statement, letter or certificate is what's needed. Say it as a limitation of what you can accept, never as an accusation.
